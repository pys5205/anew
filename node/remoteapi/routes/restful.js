...
app.get("/Hello", (req, res) => {
  urls = "http://3.39.184.25:3000/Hello";
...
app.get("/api/users", (req, res) => {
    axios
        .get('http://3.39.184.25:3000/api/users')
        .then(result => {
...
app.get("/api/users/user", (req, res) => {
    if (req.query.name == null) {
        urls = "http://3.39.184.25:3000/api/users/user?user_id="+req.query.user_id;
    } else {           
        urls = "http://3.39.184.25:3000/api/users/user?user_id="+req.query.user_id+"&name="+req.query.name;
    }
    request(urls, { json: true }, (err, result, body) => {
      if (err) { return console.log(err); }
      res.send(CircularJSON.stringify(body))
...
app.get("/api/users/:user_id", (req, res) => {
  urls = "http://3.39.184.25:3000/api/users/"+req.params.user_id;
  request(urls, { json: true }, (err, result, body) => {
      if (err) { return console.log(err); }
      res.send(CircularJSON.stringify(body))
  })
...
app.post("/api/users/userBody", (req, res) => {
    const options = {
        uri : 'http://3.39.184.25:3000/api/users/userBody',
        method : 'POST',
        form : { id: req.body.id }
    }
...
app.post("/api/users/add", (req, res) => {
    const options = {
        uri : 'http://3.39.184.25:3000/api/users/add',
        method : 'POST',
        form : { 
            id: req.body.id,
            name : req.body.name
...
app.put("/api/users/update", (req, res) => {
    const options = {
        uri : 'http://3.39.184.25:3000/api/users/update',
        method : 'PUT',
        form : { 
            id: req.body.id,
            name : req.body.name
...
app.patch("/api/user/update/:user_id", (req, res) => {
    const options = {
        uri : 'http://3.39.184.25:3000/api/user/update/'+req.params.user_id,
        method : 'PATCH',
        form : { 
            id: req.body.id,
            name : req.body.name
...
app.delete("/api/user/delete", (req, res) => {
    const options = {
        uri : 'http://3.39.184.25:3000/api/user/delete',
        method : 'DELETE',
